"""Training utilities for the Direct (DIRECT) reasoning module."""
...
def load_direct_training_data(limit: int | None = None) -> List[dspy.Example]:
    """Load examples from training_examples.jsonl in this directory."""
...
def train_direct_module(
    *,
    ...
    """Train the DirectAnswer module using GEPA if available."""

    dspy.settings.configure(lm=lm)

    trainset = load_direct_training_data(limit=num_examples)
    module = DirectAnswer()
    ...
    try:
        ...
        try:
            ...
            if "cannot schedule new futures after shutdown" in str(e):
                # Known litellm bug: https://github.com/BerriAI/litellm/issues/15366
                # This error occurs after successful optimization due to executor cleanup race condition
                if verbose:
                    logger.warning(
                        "Caught litellm executor shutdown error after successful optimization"
                    )
                optimized = (
                    module  # Use the module as-is since optimization likely completed
                )
            else:
                raise
        optimizer_name = "GEPA"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the DirectAnswer module")
    ...
    # Disk space check before LM configuration
